class AdminToolMenu extends UIScriptedMenu
{
    ...
    {
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "BreakPoint/BPSyberiaScripts/layout/AdminToolMenu.layout" );		
	...
	void UpdateMapTab(ref PluginAdminTool_MapContext context)
	{
		...
		for (int cid = 0; cid < corpseCount; cid++)
		{
			...
			m_mapWidget.AddUserMark(context.m_bodiesPositions.Get(cid), corpseName, ARGB(255, 128, 93, 128), "BreakPoint\\BPSyberiaScripts\\data\\gui\\Markers\\corpse.paa");
		}
		...
		for (int vid = 0; vid < vehiclesCount; vid++)
		{
			...
			m_mapWidget.AddUserMark(context.m_vehiclePositions.Get(vid), carName, ARGB(255, 255, 216, 0), "BreakPoint\\BPSyberiaScripts\\data\\gui\\Markers\\car.paa");
		}
		
		int playersCount = context.m_playerPositions.Count();
		for (int pid = 0; pid < playersCount; pid++)
		{
			m_mapWidget.AddUserMark(context.m_playerPositions.Get(pid), context.m_playerNames.Get(pid), ARGB(255, 255, 0, 0), "BreakPoint\\BPSyberiaScripts\\data\\gui\\Markers\\player.paa");
		}
	}
