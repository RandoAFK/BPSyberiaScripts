class SyberiaAdditionalHud
{
	...
	void CreateEspMarker(vector pos, string name, int type, int color)
	{
		...
		ref Widget w = GetGame().GetWorkspace().CreateWidgets("BreakPoint/BPSyberiaScripts/layout/AdminToolMarker.layout");
		...
		{
			iw.LoadImageFile(0, "BreakPoint\\BPSyberiaScripts\\data\\gui\\Markers\\corpse.paa");
		}
		else if (type == PluginAdminTool_EspType.VEHICLES)
		{
			iw.LoadImageFile(0, "BreakPoint\\BPSyberiaScripts\\data\\gui\\Markers\\car.paa");
